...
app.whenReady().then(() => {
  ...
  ipcMain.handle('getLastSeen', async (event, question) => {
    console.log('LastSeen for question:', question.id)
    const lastSeen = await getLastSeen(question)
    return lastSeen?.createdAt
  })

  ipcMain.handle('saveComments', async (event, comments, id) => {
    console.log('Comment received in main process:', comments, id)
